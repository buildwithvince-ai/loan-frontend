...
const OPTIONAL_DOCS = [{ key: 'gis', label: 'GIS — General Information Sheet (if corporation)' }]
...
const STORAGE_KEY = 'gr8-apply-sme'
const RESTORE_MAX_STEP = 9
    ...
    <div ref={topRef} className="min-h-screen pt-28 pb-16 px-4 sm:px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-surface/60 backdrop-blur-sm border border-border rounded-2xl p-6 sm:p-8">
          ...
          {step === 9 && (
            <Step9 docs={docs} errors={errors} handleFile={handleFile} removeFile={removeFile} />
          )}
...
function Step2({ form, set, errors }) {
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <Label required htmlFor="dtiSecNumber">
    ...
    <div className="space-y-6">
      ...
      {[1, 2, 3].map(n => (
        <div key={n} className="bg-surface-alt/20 border border-border rounded-xl p-5 space-y-4">
          <h3 className="text-blue text-sm font-semibold uppercase tracking-wider">
            {entityLabel} {n}
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <Label required htmlFor={`${prefix}${n}Name`}>
                {entityLabel} Name
              </Label>
              <Input
                id={`${prefix}${n}Name`}
                value={form[`${prefix}${n}Name`]}
                onChange={e => set(`${prefix}${n}Name`, e.target.value)}
                placeholder="Name"
              />
              <FieldError message={errors[`${prefix}${n}Name`]} />
            </div>
            <div>
              <Label required htmlFor={`${prefix}${n}Contact`}>
                Contact Person
              </Label>
              <Input
                id={`${prefix}${n}Contact`}
                value={form[`${prefix}${n}Contact`]}
                onChange={e => set(`${prefix}${n}Contact`, e.target.value)}
                placeholder="Contact person"
              />
              <FieldError message={errors[`${prefix}${n}Contact`]} />
            </div>
            <div>
              <Label required htmlFor={`${prefix}${n}Number`}>
                Contact Number
              </Label>
              <Input
                id={`${prefix}${n}Number`}
                type="tel"
                inputMode="numeric"
                value={form[`${prefix}${n}Number`]}
                onChange={e => set(`${prefix}${n}Number`, e.target.value)}
...
function Step3({ form, set, errors }) {
  return (
    <EntityListStep
      ...
      subtitle="Provide your 3 major suppliers."
    />
  )
}
...
function Step4({ form, set, errors }) {
  return (
    <EntityListStep
      ...
      subtitle="Provide your 3 major customers."
    />
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <label className="flex items-center gap-3 cursor-pointer group">
        ...
        />
        <span className="text-white text-sm font-medium group-hover:text-green transition-colors">
          Add spouse / co-borrower information
        </span>
      </label>
...
function Step10({ form, set, docs, errors }) {
  const sections = [
    ...
    },
    {
      ...
      items: [1, 2, 3].map(n => [
        `Supplier ${n}`,
        `${form[`supplier${n}Name`]} — ${form[`supplier${n}Contact`]} (${form[`supplier${n}Number`]})`,
      ]),
    },
    {
      title: 'Major Customers',
      items: [1, 2, 3].map(n => [
        `Customer ${n}`,
        `${form[`customer${n}Name`]} — ${form[`customer${n}Contact`]} (${form[`customer${n}Number`]})`,
      ]),
    },
    {
